...
def solution(user_id, banned_id):
    ul, bl = collections.defaultdict(list), collections.defaultdict(list)
    board = [[0]*len(user_id) for _ in range(len(banned_id))]
    for idx, u in enumerate(user_id):
        ul[len(u)].append((idx, u))
    for idx, b in enumerate(banned_id):
        bl[len(b)].append((idx, b))

    for l in bl.keys():
        banned, user = bl[l], ul[l]
        for idx_b, b in banned:
            for idx_u, u in user:
                for i in range(l):
                    if b[i] != '*' and b[i] != u[i]:
                        break
