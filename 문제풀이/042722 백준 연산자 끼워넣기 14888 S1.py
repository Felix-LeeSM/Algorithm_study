...
operators = [0]*a + [1]*s + [2]*m + [3]*d
candidates = set(itertools.permutations(operators, len(operators)))
for candidate in candidates:
    ret = nums[0]
    for idx in range(len(candidate)):
        if candidate[idx] == 0:
            ret += nums[idx+1]
        elif candidate[idx] == 1:
            ret -= nums[idx+1]
        elif candidate[idx] == 2:
            ret *= nums[idx+1]
        else:
            if ret < 0:
                ret = -(-ret//nums[idx+1])
                ...
                ret //= nums[idx+1]
    minimum, maximum = min(minimum, ret), max(maximum, ret)
print(maximum)
print(minimum)
