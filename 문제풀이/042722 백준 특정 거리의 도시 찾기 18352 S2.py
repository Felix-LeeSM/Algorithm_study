...
nodes, edges, k, start = map(int, input().split())
graph = {key: [] for key in range(nodes)}
for _ in range(edges):
    depart, arrive = map(int, input().split())
    graph[depart-1].append(arrive-1)
distances = [0]+[sys.maxsize for _ in range(nodes-1)]
queue = [(0, start-1)]
while queue:
    dist, depart = heapq.heappop(queue)
    if dist == k:
        continue
    for arrive in graph[depart]:
        if distances[arrive] > dist+1:
            distances[arrive] = dist+1
            heapq.heappush(queue, (dist+1, arrive))
isNone = 0
