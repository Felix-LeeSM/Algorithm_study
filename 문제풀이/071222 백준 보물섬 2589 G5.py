...
1 초	512 MB	28268	10057	7332	37.654%
문제
...
# 트리의 지름을 찾는 것처럼 아무 점이나 잡고 가장 먼 점을 찾으면 안된다....
# 트리 구조가 아닐 수 있기 때문이다.
...
queue = deque()
toVisit = [[True]*m for _ in range(n)]
for i in range(n):
    for j in range(m):
        if board[i][j] == 'L':
            temp = [k[:] for k in toVisit]
            temp[i][j] = False
            queue.append((i, j, 0))
            while queue:
                x, y, c = queue.popleft()
                for d in range(4):
                    nx, ny = x+dx[d], y+dy[d]
                    if 0 <= nx < n and 0 <= ny < m and board[nx][ny] == 'L' and temp[nx][ny]:
                        temp[nx][ny] = False
