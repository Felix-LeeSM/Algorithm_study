...
1 초	256 MB	6357	3948	3201	64.523%
문제
...
weights = int(input())
compares = int(input())
consequence = [[0]*(weights+1) for _ in range(weights+1)]
for _ in range(compares):
    a, b = map(int, input().split())
    consequence[a][b] = 1
for k in range(1, weights+1):
    for i in range(1, weights+1):
        for j in range(1, weights+1):
            if consequence[i][k] and consequence[k][j]:
                consequence[i][j] = 1
for each in range(1, weights+1):
    temp = weights-1
    for i in range(1, weights+1):
        if consequence[i][each]:
            temp -= 1
