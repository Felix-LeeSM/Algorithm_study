...
for _ in range(n):
    word = input()
    words.append(word)
    mx_l = max(mx_l, len(word))
words = [i.zfill(mx_l) for i in words]

sheet = collections.defaultdict(int)
for word in words:
    for idx, letter in enumerate(list(word)):
        if letter == '0':
            continue
        sheet[letter] += 10**(mx_l-idx-1)

par = 9
answer = 0
for mid in sorted(sheet.values(), reverse=True):
    answer += mid*par
    par -= 1
print(answer)
