...
예를 들어, 16층에 있는 민수가 0층으로 가려면 -1이 적힌 버튼을 6번, -10이 적힌 버튼을 1번 눌러
마법의 돌 7개를 소모하여 0층으로 갈 수 있습니다. 
...
setrecursionlimit(2100000000)
FILTR = 10

cheetSheet = [0, 1, 2, 3, 4, 5, 5, 4, 3, 2, 1]
...
def solution(storey):
    ...
    next = storey // FILTR
    if next < 10:
        return min(cursor + cheetSheet[next], 10 - cursor + cheetSheet[next+1])

    return min(cursor + solution(storey // FILTR), 10 - cursor + solution(storey // FILTR + 1))
