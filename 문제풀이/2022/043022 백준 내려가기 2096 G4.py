...
별표는 현재 위치이고, 
그 아랫 줄의 파란 동그라미는 원룡이가 다음 줄로 내려갈 수 있는 위치이며, 
...
N = int(input())
line_max = list(map(int, input().split()))
line_min = line_max[:]

for _ in range(N-1):
    now_max = list(map(int, input().split()))
    now_min = now_max[:]

    for i in range(3):
        now_max[i] += max(line_max[max(0, i-1):min(3, i+2)])
        now_min[i] += min(line_min[max(0, i-1):min(3, i+2)])

    line_max, line_min = now_max, now_min

print(max(line_max), min(line_min))
