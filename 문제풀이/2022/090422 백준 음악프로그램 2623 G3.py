...
1 초	128 MB	11419	5816	4593	50.126%
문제
...
for _ in range(M):
    ...
    for idx in range(n-1):
        singer, nextSinger = singers[idx], singers[idx+1]
        need[nextSinger].add(singer)
        child[singer].add(nextSinger)

ans = []
queue = deque()

for singer in range(1, N+1):
    if len(need[singer]) == 0:
        queue.append(singer)

while queue:
    canSing = queue.popleft()
    ans.append(canSing)

    for i in child[canSing]:
        need[i].discard(canSing)
        if len(need[i]) == 0:
            queue.append(i)
