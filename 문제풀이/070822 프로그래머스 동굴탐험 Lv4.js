...
// A → B, A → C (방문순서 배열 order = [...,[A,B],...,[A,C],...])
// 형태로 A를 방문 후에 방문해야 할 방이 B와 C로 두 개 또는 그 이상인 경우
// X → A, Z → A (방문순서 배열 order = [...,[X,A],...,[Z,A],...])
// 형태로 A를 방문하기 전에 방문해야 할 방이 X와 Z로 두 개 또는 그 이상인 경우
// A → B → C (방문순서 배열 order = [...,[A,B],...,[B,C],...)
// 형태로 B처럼 A 방문 후이면서 동시에 C 방문 전인 경우

// 그리고 먼저 방문해야 할 방과 나중에 방문할 방을 반드시 연속해서 방문해야 할 필요는 없어
// A방을 방문한 후 다른 방을 방문한 후 B방을 방문해도 좋습니다.
...
// https://school.programmers.co.kr/learn/courses/30/lessons/67260?language=javascript

function solution(n, path, order) {
  const adjacent = Array(n)
    .fill(null)
    .map((_) => []);
  ...
  // condition의 idx를 가려면, condition[idx]를 먼저 가야함
  for (const [first, second] of order) {
    condition.set(second, first);
  }

  // 0에서 시작하는데 0을 가려면 다른 곳을 먼저 방문해야하면 false
  if (condition.get(0)) return false;
  ...
  while (stack.length) {
    ...
    if (
      condition.get(current) !== undefined &&
      !visited[condition.get(current)]
    ) {
      // current를 가려면, condition.get(current)를 먼저 가야함
      // 먼저 가야하는 곳이 있고, 방문하지 않았으면 나중을 위해 저장해둠
      kept.set(condition.get(current), current);
      continue;
    }
    ...
    if (kept.get(current) !== undefined) {
      // current를 감으로써 갈 수 있게 된 kept[current]가 있느냐?
      stack.push(kept.get(current));
    }
  }

  return visited.reduce((total, curr) => total && curr);
}
console.log(
  solution(
    9,
