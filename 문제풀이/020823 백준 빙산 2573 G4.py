...
1 초	256 MB	55889	15750	10452	25.760%
문제
...
def solution(N: int, M: int, board: list[list[int]]) -> int:
            ...
            for j in range(1, M-1):
                if board[i][j]:
                    for d in range(4):
                        di, dj = vctr[d]
                        ni, nj = i + di, j + dj
                        deltas[i][j] += board[ni][nj] == 0

        cnt = 0
        for i in range(1, N-1):
            for j in range(1, M-1):
                if board[i][j]:
                    board[i][j] = max(0, board[i][j] - deltas[i][j])
                    cnt += board[i][j] != 0

        return cnt

    def is_broken(N: int, M: int, board: list[list[int]], cnt: int) -> bool:
        # 빙산이 분리되었는지 확인한다.
        ...
        while stack:
            ...
            for d in range(4):
                di, dj = vctr[d]
                ni, nj = i + di, j + dj
                if visited[ni][nj] or not board[ni][nj]:
                    continue
    ...
    time = 0
    while True:
        time, cnt = time+1, elapse()

        if not cnt:
            return 0

        if is_broken(N, M, board, cnt):
            return time
