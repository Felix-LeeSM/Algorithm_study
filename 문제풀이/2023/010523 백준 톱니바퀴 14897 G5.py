...
2 초	512 MB	29378	15797	11545	54.335%
문제
...
LEFT = 6
CLOCK_WISE = 1
REVERSE_CLOCK_WISE = -1
N = '0'
S = '1'


# deque의 rotate는 오른쪽으로 민다.
def rotate(gears, idx, rotation_direction):
    dp = [0]*len(gears)
    dp[idx] = rotation_direction

    if idx < len(gears) - 1:
        mark(gears, idx+1, dp, rotation_direction*-1, RIGHT)
    if idx:
        mark(gears, idx-1, dp, rotation_direction*-1, LEFT)

    for dir, gear in zip(dp, gears):
        if dir == CLOCK_WISE:
            gear.rotate(1)
        elif dir == REVERSE_CLOCK_WISE:
            gear.rotate(-1)


def mark(gears, idx, dp, rotation_direction, spread_direction):
    if spread_direction == LEFT:
        gear_left, gear_right = gears[idx], gears[idx+1]
        if will_rotate(gear_left, gear_right):
            dp[idx] = rotation_direction
            if idx:
                mark(gears, idx-1, dp, rotation_direction*-1, spread_direction)

    if spread_direction == RIGHT:
        gear_left, gear_right = gears[idx-1], gears[idx]
        if will_rotate(gear_left, gear_right):
            dp[idx] = rotation_direction
            if idx < len(gears)-1:
                mark(gears, idx+1, dp, rotation_direction*-1, spread_direction)
...
def will_rotate(left_gear, right_gear):
    return left_gear[RIGHT] != right_gear[LEFT]


def score(gears):
    return sum([(gear[TOP] == S) * 2 ** (expo) for expo, gear in enumerate(gears)])
