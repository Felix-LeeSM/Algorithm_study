...
# 크루스칼 알고리즘으로 최소 신장 트리를 구했다.
# 그런데, 크루스칼 알고리즘을 적용하기 위해서는 전처리가 필요하다.
# dfs로 전체 탐색을 하면서 각 층계에 대해서 넣어줬다.
# 어차피 마주할 때는 두번 체크하니까 영역의 번호가 더 작은 것을 만났을 때만 edge를 찾았다.
# 전처리를 하고 유니온 파인드로 구했다.
