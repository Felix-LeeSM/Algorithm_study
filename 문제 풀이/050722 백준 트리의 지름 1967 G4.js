...
for (const [p, c, d] of edges) {
  Children[p - 1].push([c - 1, d])
}

const pre = (node) => {
  for (const [child, d] of Children[node]) {
    pre(child)
    if (board[node][1] < board[child][1] + d) {
      board[node][2] = board[node][1]
      board[node][1] = board[child][1] + d
    } else {
      board[node][2] = Math.max(board[node][2], board[child][1] + d)
    }
  }
}
...
const post = (node) => {
  ...
  for (const [child, d] of Children[node]) {
    post(child)
    board[node][0] = Math.max(board[node][0], board[child][0])
  }
}
