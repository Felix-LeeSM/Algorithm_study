...
["img12.png", "img10.png", "img2.png", "img1.png"]일 경우, 일반적인 정렬은 
["img1.png", "img10.png", "img12.png", "img2.png"] 순이 되지만, 숫자 순으로 정렬된 
["img1.png", "img2.png", "img10.png", img12.png"] 순이 훨씬 자연스럽다.

무지는 단순한 문자 코드 순이 아닌, 파일명에 포함된 숫자를 반영한 정렬 기능을 저장소 관리 프로그램에 구현하기로 했다.

소스 파일 저장소에 저장된 파일명은 100 글자 이내로, 영문 대소문자, 숫자, 공백(" "), 
마침표("."), 빼기 부호("-")만으로 이루어져 있다. 파일명은 영문자로 시작하며, 숫자를 하나 이상 포함하고 있다.
    ...
    for i in range(len(files)):
        for j in range(1, len(files[i])):
            if files[i][j].isdigit():
                k = j
                while k < j+5 and k < len(files[i]) and files[i][k].isdigit():
                    # 숫자는 최대 5개이며, tail이 아예 없을 수도 있다.
                    k += 1
                else:
                    seperation.append(
                        [files[i][:j], files[i][j:k], files[i][k:]])
                    break
    files = [''.join(i) for i in sorted(
        seperation, key=lambda x:(x[0].lower(), int(x[1])))]
    return files
