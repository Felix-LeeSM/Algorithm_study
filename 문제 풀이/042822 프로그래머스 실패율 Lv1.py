...
def solution(N, stages):
    ...
    challenged = collections.defaultdict(int)
    clear = collections.defaultdict(int)
    failure = []
    for stage in stages:
        users[stage] += 1
        clear[stage-1] += 1 if stage-1 else 0
        challenged[stage] += 1
    for i in range(N, 0, -1):
        clear[i] += clear[i+1]
        challenged[i] += challenged[i+1]
    for i in range(1, N+1):
        failure.append(clear[i]/challenged[i] if challenged[i] else 1)
    failure = zip(failure, list(range(1, N+1)))
    return [i[1] for i in sorted(failure)]
