...
def dj(fr, to):  # fr 에서 to 까지의 최단 거리
    if fr == to:
        return 0
    dist = [inf]*(n+1)
    dist[fr] = 0
    ...
    while que:
        ...
        for end, di in graph[st]:
            if dist[end] > d+di:
                dist[end] = d+di
                heapq.heappush(que, (d+di, end))
    return dist[to]


bridge = dj(mid2, mid1)
short_one = min(dj(1, mid1) + dj(mid2, n), dj(1, mid2) + dj(mid1, n))
print(-1 if short_one == inf or bridge == inf else short_one+bridge)
