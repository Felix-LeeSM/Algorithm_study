...
# 구현 문제이다.
# 문제를... 잘 읽어야 한다...
# 방향 전환은 해당 초가 끝난 후에 이루어진다........
# 방향 전환은 vector를 이용해 구현하였다.
# 좌, 우, 뒤돌기 정도는 규칙을 잘 주어 vector를 짜면 쉽게 구현할 수 있을 듯 하다.
# 사실 apple을 제거하는 과정을 더 빠르게 하는 방법을 찾고 싶었다.
...
apples = list()
for _ in range(K):
    each = tuple(map(int, read().split()))
    apples.append((each[0]-1, each[1]-1))
L = int(read())
moves = []
for _ in range(L):
